...
def create_token(
    token: TokenConfigurationCreate,
    db: Session = Depends(get_db)
):
    """Create a new token configuration."""
    db_token = TokenConfiguration(**token.model_dump())
    db.add(db_token)
    db.commit()
    db.refresh(db_token)
    return db_token
...
async def get_token_old(
    ...
    """
    return token_service.get_token_configuration(db, token_id, wallet)
